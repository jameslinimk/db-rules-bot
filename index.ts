...
let begin: false | NodeJS.Timer = false
...
client.on("messageCreate", async (message) => {
    if (!guild || message.guild.id !== guild.id || message.author.bot || !message.content.startsWith(prefix)) return
    ...
    switch (command) {
        case "begin":
            ...
            begin = setInterval(async () => {
                ...
                const gif = await getGif(`rule ${current}`)
